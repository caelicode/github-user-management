...
def main():
    ...
    callback_delay = int(os.environ.get("CALLBACK_DELAY", "120"))  # Delay in seconds, default 120s
    ...
    logging.info(f"Processing {len(valid_usernames)} users in {'TEST' if test_mode else 'LIVE'} mode")
    ...
    if callback_token:
        ...
        # Add delay before sending callback to avoid race conditions
        logging.info(f"Waiting {callback_delay} seconds before sending callback...")
        time.sleep(callback_delay)
        
        # Send callback
        logging.info("Sending callback to update user lists in cdcent/ocio-github-infra")
